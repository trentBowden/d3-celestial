...
Celestial1st.display = function(config) {
  var parentElement, animationID,
      container = Celestial1st.container,
      animations = [],
      current = 0,
      repeat = false;
  ...
  mapProjection = Celestial1st.projection(cfg.projection).rotate(rotation).translate([canvaswidth/2, canvasheight/2]).scale(scale * zoomlevel);
  ...
  if ($("loc") === null) geo(cfg);
  else if (cfg.location === true && cfg.follow === "zenith") rotate({center: Celestial1st.zenith()});
  ...
  function load() {
    ...
    d3.json(path + filename("constellations"), function(error, json) {
      ...
      Celestial1st.constellations = getConstellationList(con);
      redraw();
    });
    ...
    if (Celestial1st.data.length > 0) {
      Celestial1st.data.forEach( function(d) {
        if (has(d, "file")) d3.json(d.file, d.callback);
        else setTimeout(d.callback, 0);
      }, this);
    }

    if (cfg.lang && cfg.lang != "") apply(Celestial1st.setLanguage(cfg.lang));
    //redraw();
  }
  ...
  function reproject(config) {
    ...
    var rot = mapProjection.rotate(), ctr = mapProjection.center(), sc = mapProjection.scale(), ext = zoom.scaleExtent(), clip = [],
        prjFrom = Celestial1st.projection(cfg.projection).center(ctr).translate([canvaswidth/2, canvasheight/2]).scale([ext[0]]),
    ...
    var prjTo = Celestial1st.projection(config.projection).center(ctr).translate([canvaswidth/2, canvaswidth/prj.ratio/2]).scale([prj.scale * width/1024]);
    ...
    d3.select({}).transition().duration(interval).tween("projection", function () {
      ...
      mapProjection = Celestial1st.projection(config.projection).rotate(rot).translate([canvaswidth/2, canvasheight/2]).scale(scale * zoomlevel);
      map.projection(mapProjection);
      setClip(projectionSetting.clip);
  ...
  function redraw() {
    ...
    if (cfg.constellations.bounds) {
      container.selectAll(".boundaryline").each(function(d) {
        ...
        if (Celestial1st.constellation) {
          var re = new RegExp("\\b" + Celestial1st.constellation + "\\b");
      ...
      container.selectAll(".constname").each( function(d) {
        if (clip(d.geometry.coordinates)) {
          ...
          textCollision = !!constellationsNamesSizeObjects.find(function(constellationName) {
            return Celestial1st.helpers.checkTextCollistion(constellationName, sizesObj, true);
          });

          // if (!textCollision && Celestial1st.helpers.checkTextInsideCircle(sizesObj, cr, cr, cr * constellationThreshold)) {
    ...
    if ((cfg.location || cfg.formFields.location) && cfg.planets.show && Celestial1st.origin) {
      var dt = Celestial1st.date(),
          o = Celestial1st.origin(dt).spherical();
            ...
            do {
              ...
              try {
                planetLabelObjects.forEach(function (bookedSize) {
                  var tempCheckStandardLabelLocation = Celestial1st.helpers.checkTextCollistion(bookedSize, sizesObj, false);
              ...
              // if (Celestial1st.helpers.checkTextInsideCircle(sizesObj, cr, cr, cr * planetsThreshold)) {
    ...
    Celestial1st.helpers = {
      ...
      checkTextInsideCircle: function(sizesObj, cx, cy, r) {
        var isCollisionA = Celestial1st.helpers.pointInCircle(
            sizesObj.x + sizesObj.width,
            sizesObj.y,
            cx, cy, r
        );
        var isCollisionB = Celestial1st.helpers.pointInCircle(
            sizesObj.x + sizesObj.width,
            sizesObj.y + sizesObj.height,
            cx, cy, r
        );
        var isCollisionC = Celestial1st.helpers.pointInCircle(
            sizesObj.x,
            sizesObj.y + sizesObj.height,
            cx, cy, r
        );
        var isCollisionD = Celestial1st.helpers.pointInCircle(
            sizesObj.x,
            sizesObj.y,
            cx, cy, r
        );
      ...
      pointInRectangle: function(x1, y1, x2, y2, x3, y3, x4, y4, x, y) {
        ...
        y4 = Math.round(y4);
        y = Math.round(y);
        var A = (Celestial1st.helpers.area(x1, y1, x2, y2, x3, y3) +
            Celestial1st.helpers.area(x1, y1, x4, y4, x3, y3));
        var A1 = Celestial1st.helpers.area(x, y, x1, y1, x2, y2);
        var A2 = Celestial1st.helpers.area(x, y, x2, y2, x3, y3);
        var A3 = Celestial1st.helpers.area(x, y, x3, y3, x4, y4);
        var A4 = Celestial1st.helpers.area(x, y, x1, y1, x4, y4);

        return (A === A1 + A2 + A3 + A4);
      },
      checkTextCollistion: function(bookedSize, sizesObj, returnBoolean) {
         var isCollisionA = Celestial1st.helpers.pointInRectangle(
        ...
        var isCollisionB = Celestial1st.helpers.pointInRectangle(
            bookedSize.x +  bookedSize.width,
            bookedSize.y,
            bookedSize.x +  bookedSize.width,
            bookedSize.y + bookedSize.height,
        ...
        var isCollisionC = Celestial1st.helpers.pointInRectangle(
            bookedSize.x +  bookedSize.width,
            bookedSize.y,
            bookedSize.x +  bookedSize.width,
            bookedSize.y + bookedSize.height,
        ...
        var isCollisionD = Celestial1st.helpers.pointInRectangle(
            bookedSize.x +  bookedSize.width,
            bookedSize.y,
            bookedSize.x +  bookedSize.width,
            bookedSize.y + bookedSize.height,
    ...
    if ((cfg.location || cfg.formFields.location) && cfg.horizon.show && !projectionSetting.clip) {
      circle.origin(Celestial1st.nadir());
    ...
    if (hasCallback) {
      Celestial1st.runCallback();
    }

    //Celestial1st.updateForm();
  ...
  this.reload = function(config) {
    ...
    if (cfg.follow === "center" && has(cfg, "center")) {
      ...
      ctr = getAngles(Celestial1st.zenith());
    }
  ...
  /* obsolete
  if (!has(this, "date"))
    this.date = function() { console.log("Celestial1st.date() needs config.location = true to work." ); };
  */
  load();
};
...
Celestial1st.projection = function(projection) {
  var p, raw, forward;
...
Celestial1st.eulerAngles = function () { return eulerAngles; };
Celestial1st.poles = function () { return poles; };
...
euler.init();
Celestial1st.euler = function () { return euler; };
...
Celestial1st.horizontal = horizontal;
Celestial1st.ha = function(dt, lng, ra) {
...
Celestial1st.add = function(dat) {
  ...
  Celestial1st.data.push(res);
};

Celestial1st.remove = function(i) {
  if (i !== null && i < Celestial1st.data.length) {
    return Celestial1st.data.splice(i,1);
  }
};

Celestial1st.clear = function() {
  Celestial1st.data = [];
};

Celestial1st.addCallback = function(dat) {
  Celestial1st.callback = dat;
  hasCallback = (dat !== null);
};

Celestial1st.runCallback = function(dat) {
  hasCallback = false; // avoid recursion
  Celestial1st.callback();
  hasCallback = true;
};
...
function getPlanet(id, dt) {
  dt = dt || Celestial1st.date();
  if (!Celestial1st.origin) return;

  var o = Celestial1st.origin(dt).spherical(), res;
     
  Celestial1st.container.selectAll(".planet").each(function(d) {
    if (id === d.id()) {
...
Celestial1st.getData = getData;
Celestial1st.getPoint = getPoint;
Celestial1st.getPlanet = getPlanet;
...
Celestial1st.settings = function () { return settings; };
...
Celestial1st.projections = function () { return projections; };
...
function $(id,applyPrefix) {
  if (typeof applyPrefix === 'undefined' || applyPrefix === null || applyPrefix === true) {
    return document.getElementById(Celestial1st.iId+id);
  } else {
...
function form(cfg) {
  ...
  var prj = Celestial1st.projections(), leo = Celestial1st.eulerAngles();
  var div = d3.select("#"+Celestial1st.iId+"celestial-form");
  //if div doesn't exist, create it
  if (div.size() < 1) {
    var container = (config.container || "celestial-map");
    div = d3.select("#" + container).select(function() { return this.parentNode; }).append("div").attr("id", Celestial1st.iId+"celestial-form");
  }
  var ctrl = div.append("div").attr("class", "ctrl");
  var frm = ctrl.append("form").attr("id", Celestial1st.iId+"params").attr("name", "params").attr("method", "get").attr("action" ,"#");
  
  //Map parameters    
  var col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"general");
  ...
  var sel = col.append("select").attr("id", Celestial1st.iId+"projection").on("change", reproject);
  var selected = 0;
  ...
  sel = col.append("select").attr("id", Celestial1st.iId+"transform").on("change", reload);
  ...
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"centerx").attr("title", "Center right ascension/longitude").attr("max", "24").attr("min", "0").attr("step", "0.1").on("change", turn);
  col.append("span").attr("id",  Celestial1st.iId+"cxunit").html("h");
  //addList("centerx", "ra");
  
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"centery").attr("title", "Center declination/latitude").attr("max", "90").attr("min", "-90").attr("step", "0.1").on("change", turn);
  col.append("span").html("\u00b0");

  col.append("label").attr("title", "Orientation").attr("for", "centerz").html("Orientation");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"centerz").attr("title", "Center orientation").attr("max", "180").attr("min", "-180").attr("step", "0.1").on("change", turn);
  col.append("span").html("\u00b0");

  col.append("label").attr("for", "orientationfixed").attr("class", "advanced").html("Fixed");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"orientationfixed").attr("class", "advanced").property("checked", config.orientationfixed).on("change", apply);

  col.append("label").attr("title", "Center and zoom in on this constellation").attr("for", "constellation").html("Show");
  col.append("select").attr("id", Celestial1st.iId+"constellation").on("change", showConstellation);
  
  setCenter(config.center, config.transform);

  // Stars 
  col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"stars");
  
  col.append("label").attr("class", "header").attr("for", "stars-show").html("Stars");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"stars-show").property("checked", config.stars.show).on("change", apply);
  
  col.append("label").attr("for", "stars-limit").html("down to magnitude");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"stars-limit").attr("title", "Star display limit (magnitude)").attr("value", config.stars.limit).attr("max", "6").attr("min", "-1").attr("step", "0.1").on("change", apply);
  
  col.append("label").attr("for", "stars-colors").html("with spectral colors");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"stars-colors").property("checked", config.stars.colors).on("change", apply);
  
  col.append("label").attr("for", "stars-color").html("or default color ");
  col.append("input").attr("type", "color").attr("autocomplete", "off").attr("id", Celestial1st.iId+"stars-style-fill").attr("title", "Star color").property("value", config.stars.style.fill).on("change", apply);
  ...
  for (var fld in names) {
    ...
    if (keys.length > 1) {
      ...
      selected = 0;
      col.append("label").attr("title", "Type of star name").attr("id", Celestial1st.iId+"label-propername").attr("for", "stars-" + fld + "Type").html(function () { return fld === "propername" ? "proper names" : ""; });
      sel = col.append("select").attr("id", Celestial1st.iId+"stars-" + fld + "Type").attr("class", function () { return fld === "propername" ? "advanced" : ""; }).on("change", apply);
      ...
      col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"stars-" + fld).property("checked", config.stars[fld]).on("change", apply);
    ...
    col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"stars-" + fld + "Limit").attr("title", "Star name display limit (magnitude)").attr("value", config.stars[fld + "Limit"]).attr("max", "6").attr("min", "-1").attr("step", "0.1").on("change", apply);
  ...
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"stars-size").attr("class", "advanced").attr("title", "Size of the displayed star disk; base").attr("value", config.stars.size).attr("max", "100").attr("min", "0").attr("step", "0.1").on("change", apply);

  col.append("label").attr("for", "stars-exponent").attr("class", "advanced").html(" * e ^ (exponent");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"stars-exponent").attr("class", "advanced").attr("title", "Size of the displayed star disk; exponent").attr("value", config.stars.exponent).attr("max", "3").attr("min", "-1").attr("step", "0.01").on("change", apply);
  col.append("span").attr("class", "advanced").text(" * (magnitude + 2))  [* adaptation]");
  
  enable($("stars-show"));
  
  // DSOs 
  col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"dsos");
  
  col.append("label").attr("class", "header").attr("title", "Deep Space Objects").attr("for", "dsos-show").html("DSOs");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"dsos-show").property("checked", config.dsos.show).on("change", apply);
  
  col.append("label").attr("for", "dsos-limit").html("down to mag");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"dsos-limit").attr("title", "DSO display limit (magnitude)").attr("value", config.dsos.limit).attr("max", "6").attr("min", "0").attr("step", "0.1").on("change", apply);


  col.append("label").attr("for", "dsos-colors").html("with symbol colors");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"dsos-colors").property("checked", config.dsos.colors).on("change", apply);
  
  col.append("label").attr("for", "dsos-color").html("or default color ");
  col.append("input").attr("type", "color").attr("autocomplete", "off").attr("id", Celestial1st.iId+"dsos-style-fill").attr("title", "DSO color").property("value", config.dsos.style.fill).on("change", apply);
  ...
  for (fld in names) {
    ...
    sel = col.append("select").attr("id", Celestial1st.iId+"dsos-" + fld + "Type").attr("class", "advanced").on("change", apply);
    ...
    col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"dsos-" + fld).property("checked", config.dsos[fld]).on("change", apply);
  }    
    
  col.append("label").attr("for", "dsos-nameLimit").html("down to mag");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"dsos-nameLimit").attr("title", "DSO name display limit (magnitude)").attr("value", config.dsos.nameLimit).attr("max", "6").attr("min", "0").attr("step", "0.1").on("change", apply);
  col.append("br");

  col.append("label").attr("for", "dsos-size").attr("class", "advanced").html("DSO symbol size: (base");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"dsos-size").attr("class", "advanced").attr("title", "Size of the displayed symbol: base").attr("value", config.dsos.size).attr("max", "100").attr("min", "0").attr("step", "0.1").on("change", apply);

  col.append("label").attr("for", "dsos-exponent").attr("class", "advanced").html(" * 2 [* adaptation] - magnitude) ^ exponent");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"dsos-exponent").attr("class", "advanced").attr("title", "Size of the displayed symbol; exponent").attr("value", config.dsos.exponent).attr("max", "3").attr("min", "-1").attr("step", "0.01").on("change", apply);

  enable($("dsos-show"));

  // Constellations 
  col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"constellations");
  col.append("label").attr("class", "header").html("Constellations");
  //col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"constellations-show").property("checked", config.constellations.show).on("change", apply);
  ...
  for (fld in names) {
    ...
    if (nameKeys.length > 1) {
      ...
      sel = col.append("select").attr("id", Celestial1st.iId+"constellations-" + fld + "Type").attr("class", "advanced").on("change", apply);
      ...
      col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"constellations-" + fld).property("checked", config.constellations[fld]).on("change", apply);
    } else if (nameKeys.length === 1) {
      //Simple field
      col.append("label").attr("for", "constellations-" + fld).attr("class", "advanced").html(" " + names[fld][nameKeys[0]]);
      col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"constellations-" + fld).attr("class", "advanced").property("checked", config.constellations[fld]).on("change", apply);
    }      
  }
  col.append("label").attr("for", "constellations-lines").html(" lines");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"constellations-lines").property("checked", config.constellations.lines).on("change", apply);
  
  col.append("label").attr("for", "constellations-bounds").html(" boundaries");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"constellations-bounds").property("checked", config.constellations.bounds).on("change", apply);
  ...
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"lines-graticule-show").property("checked", config.lines.graticule.show).on("change", apply);
  
  col.append("label").attr("for", "lines-equatorial").html("Equator");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"lines-equatorial-show").property("checked", config.lines.equatorial.show).on("change", apply);
  
  col.append("label").attr("for", "lines-ecliptic").html("Ecliptic");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"lines-ecliptic-show").property("checked", config.lines.ecliptic.show).on("change", apply);
  
  col.append("label").attr("for", "lines-galactic").html("Galactic plane");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"lines-galactic-show").property("checked", config.lines.galactic.show).on("change", apply);
  
  col.append("label").attr("for", "lines-supergalactic").html("Supergalactic plane");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"lines-supergalactic-show").property("checked", config.lines.supergalactic.show).on("change", apply);

  // Other
  col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"other");
  col.append("label").attr("class", "header").html("Other");
  
  col.append("label").attr("for", "mw-show").html("Milky Way");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"mw-show").property("checked", config.mw.show).on("change", apply);
  
  col.append("label").attr("for", "mw-style-fill").attr("class", "advanced").html(" color");
  col.append("input").attr("type", "color").attr("id", Celestial1st.iId+"mw-style-fill").attr("class", "advanced").attr("title", "Milky Way color").attr("value", config.mw.style.fill).on("change", apply);

  col.append("label").attr("for", "mw-style-opacity").attr("class", "advanced").html(" opacity");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"mw-style-opacity").attr("class", "advanced").attr("title", "Transparency of each Milky Way layer").attr("value", config.mw.style.opacity).attr("max", "1").attr("min", "0").attr("step", "0.01").on("change", apply);
  
  col.append("label").attr("for", "advanced").html("Advanced options");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"advanced").property("checked", config.advanced).on("change", apply);
  
  col.append("br");
  
  col.append("label").attr("for", "background-fill").html("Background color");
  col.append("input").attr("type", "color").attr("id", Celestial1st.iId+"background-fill").attr("title", "Background color").attr("value", config.background.fill).on("change", apply);

  col.append("label").attr("for", "background-opacity").attr("class", "advanced").html("opacity");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"background-opacity").attr("class", "advanced").attr("title", "Background opacity").attr("value", config.background.opacity).attr("max", "1").attr("min", "0").attr("step", "0.01").on("change", apply);
  ...
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"adaptable").attr("class", "advanced").property("checked", config.adaptable).on("change", apply);
  ...
  sel = col.append("select").attr("id", Celestial1st.iId+"lang").on("change", apply);
  ...
  col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"download");
  col.append("label").attr("class", "header").html("Download");

  col.append("input").attr("type", "button").attr("id", Celestial1st.iId+"download-png").attr("value", "PNG Image").on("click", function() {
  ...
  col.append("input").attr("type", "button").attr("id", Celestial1st.iId+"download-svg").attr("value", "SVG File").on("click", function() {
  ...
  function showCon(id) {
    var z, anims = [],
        config = globalConfig;
    if (id === "---") { 
      Celestial1st.constellation = null;
      z = Celestial1st.zoomBy();
      if (z !== 1) anims.push({param:"zoom", value:1/z, duration:0});
      Celestial1st.animate(anims, false);    
      //Celestial1st.redraw();
      return;
    }
    if (!isObject(Celestial1st.constellations) || !has(Celestial1st.constellations, id)) return;
    
    var con = Celestial1st.constellations[id];
  ...
  function update() {
    ...
    d3.selectAll("#"+Celestial1st.iId+"celestial-form input, #"+Celestial1st.iId+"celestial-form select").each( function(d, i) {
  ...
  function get(id) {
    var a = id.split("-");
    a[0] = a[0].replace(Celestial1st.iId, '');
    switch (a.length) {
  ...
  Celestial1st.updateForm  = update;
  Celestial1st.showConstellation = showCon;
  Celestial1st.setLanguage = function(lang) {
...
function popError(nd, err) {
  var p = findPos(nd);
  d3.select("#"+Celestial1st.iId+"error").html(err).style( {top:px(p[1] + nd.offsetHeight + 1), left:px(p[0]), opacity:1} );
  nd.focus();
}
...
function testNumber(node) {
  ...
  d3.select("#"+Celestial1st.iId+"error").style( {top:"-9999px", left:"-9999px", opacity:0} );
  return true; 
}
...
function showAdvanced(showit) {
  ...
  d3.selectAll("#"+Celestial1st.iId+"label-propername").style("display", showit ? "none" : "inline-block");
}
...
function setVisibility(cfg, which) {
   ...
   if (cfg.form === false && cfg.location === true) {
     d3.select("#"+Celestial1st.iId+"celestial-form").style("display", "inline-block");
     for (fld in cfg.formFields) {
      if (!has(cfg.formFields, fld)) continue;
       if (fld === "location") continue;
       d3.select("#"+Celestial1st.iId + fld).style( {"display": "none"} );
     }
     return;
   }
   // hide if not desired
   if (cfg.form === false) d3.select("#"+Celestial1st.iId+"celestial-form").style("display", "none");
...
function listConstellations() {
  var sel = d3.select("#"+Celestial1st.iId+"constellation"),
      list = [], selected = 0, id, name, config = globalConfig;
    
  Celestial1st.container.selectAll(".constname").each( function(d, i) {
    id = d.id;
  ...
  //Celestial1st.constellations = list;
}
...
Celestial1st.locationAndDateIntoCenter = function(geopos, date, transform) {
  ...
  if (!isNaN(lon) && !isNaN(lat)) {
    ...
    return Celestial1st.getPoint(inverse, transform);
  }
};
...
function geo(cfg) {
  var dtFormat = d3.time.format("%Y-%m-%d %H:%M:%S"),
      ...
      frm = d3.select("#"+Celestial1st.iId+"celestial-form form").insert("div", "div#general").attr("id", "loc");
  ...
  var col = frm.append("div").attr("class", "col").attr("id", Celestial1st.iId+"location").style("display", "none");
  //Latitude & longitude fields
  col.append("label").attr("title", "Location coordinates long/lat").attr("for", "lat").html("Location");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"lat").attr("title", "Latitude").attr("placeholder", "Latitude").attr("max", "90").attr("min", "-90").attr("step", "0.0001").attr("value", geopos[0]).on("change",  function () {
    if (testNumber(this) === true) go();
  });
  col.append("span").html("\u00b0");
  col.append("input").attr("type", "number").attr("id", Celestial1st.iId+"lon").attr("title", "Longitude").attr("placeholder", "Longitude").attr("max", "180").attr("min", "-180").attr("step", "0.0001").attr("value", geopos[1]).on("change",  function () {
    if (testNumber(this) === true) go();
  });
  col.append("span").html("\u00b0");
  //Here-button if supported
  if ("geolocation" in navigator) {
    col.append("input").attr("type", "button").attr("value", "Here").attr("id", Celestial1st.iId+"here").on("click", here);
  }
  //Datetime field with dtpicker-button
  col.append("label").attr("title", "Local date/time").attr("for", "datetime").html(" Date/time");
  col.append("input").attr("type", "button").attr("id", Celestial1st.iId+"day-left").attr("title", "One day back").on("click", function () {
    date.setDate(date.getDate() - 1);
    $("datetime").value = dateFormat(date, timeZone);
    go();
  });
  col.append("input").attr("type", "text").attr("id", Celestial1st.iId+"datetime").attr("title", "Date and time").attr("value", dateFormat(date, timeZone))
  .on("click", showpick, true).on("input", function () {
    this.value = dateFormat(date, timeZone);
    if (!dtpick.isVisible()) showpick();
  });
  col.append("div").attr("id", Celestial1st.iId+"datepick").on("click", showpick);
  col.append("input").attr("type", "button").attr("id", Celestial1st.iId+"day-right").attr("title", "One day forward").on("click", function () {
    date.setDate(date.getDate() + 1);
    $("datetime").value = dateFormat(date, timeZone);
    go();
  });
  //Now -button sets current time & date of device
  col.append("input").attr("type", "button").attr("value", "Now").attr("id", Celestial1st.iId+"now").on("click", now);
  //Horizon marker
  col.append("br");
  col.append("label").attr("title", "Show horizon marker").attr("for", "horizon-show").html(" Horizon marker");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"horizon-show").property("checked", config.horizon.show).on("change", apply);
  //Daylight
  col.append("label").attr("title", "Show daylight").attr("for", "daylight-show").html("Daylight sky");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"daylight-show").property("checked", config.daylight.show).on("change", apply);col.append("br");

  //Show planets
  col.append("label").attr("title", "Show solar system objects").attr("for", "planets-show").html(" Planets, Sun & Moon");
  col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"planets-show").property("checked", config.planets.show).on("change", apply);
  //Planet names
  ...
  for (var fld in names) {
    ...
    if (keys.length > 1) {
      ...
      var sel = col.append("select").attr("id", Celestial1st.iId+"planets-" + fld + "Type").on("change", apply);
      ...
      if (fld === "names") {
        ...
        col.append("input").attr("type", "checkbox").attr("id", Celestial1st.iId+"planets-" + fld).property("checked", config.planets[fld]).on("change", apply);
      }
    }
  }
  ...
  d3.select(document).on("mousedown", function () {
    if (!hasParent(d3.event.target, Celestial1st.iId+"celestial-date") && dtpick.isVisible()) dtpick.hide();
  });
  ...
  function go() {
    ...
    if (!isNaN(lon) && !isNaN(lat)) {
      ...
      var dtc = new Date(date.valueOf() - (timeZone - localZone) * 60000);
      zenith = Celestial1st.getPoint(horizontal.inverse(dtc, [90, 0], geopos), config.transform);
      zenith[2] = 0;
      if (config.follow === "zenith") {
        Celestial1st.rotate({center:zenith});
      } else {
        Celestial1st.redraw();
      }
    }
  ...
  Celestial1st.dateFormat = dateFormat;

  Celestial1st.date = function (dt, tz) {
    if (!dt) return date;
  ...
  Celestial1st.timezone = function (tz) {
  ...
  Celestial1st.position = function () { return geopos; };
  Celestial1st.location = function (loc, tz) {
  ...
  Celestial1st.skyview = function (cfg) {
    ...
    else Celestial1st.redraw();
  };
  Celestial1st.dtLoc = Celestial1st.skyview;
  Celestial1st.zenith = function () { return zenith; };
  Celestial1st.nadir = function () {
    var b = -zenith[1],
        l = zenith[0] + 180;
    if (l > 180) l -= 360;
    return [l, b-0.001];
  ...
  if (has(cfg, "formFields") && (cfg.location === true || cfg.formFields.location === true)) {
    d3.select(Celestial1st.iId+"#location").style( {"display": "inline-block"} );
  }
  //only if appropriate
  if (cfg.location === true && cfg.formFields.location === true)
    setTimeout(go, 1000);
...
Celestial1st.graticule = function(svg, path, trans) {
...
Celestial1st.exportSVG = function(fname) {
  var versionTitle = "PositivePrints ver 1.5";
  d3.select('#d3-celestial-svg').remove();

  var doc = d3.select("body").append("div").attr("id", Celestial1st.iId+"d3-celestial-svg").attr("style", "display: none");

  var svg = d3.select("#"+Celestial1st.iId+"d3-celestial-svg").append("svg"), //.attr("style", "display: none"),
      m = Celestial1st.metrics(),
      cfg = settings.set(),
      path = cfg.datapath,
      ...
      projection = Celestial1st.projection(cfg.projection).rotate(rotation).translate([m.width/2, m.height/2]).scale([m.scale]),
...
*/
  ...
  if (cfg.lines.graticule.show) {
    if (cfg.transform === "equatorial") {
      ...
      Celestial1st.graticule(groups.gridLines, map, cfg.transform);
      styles.gridLines = svgStyle(cfg.lines.graticule);
    }
    if (has(cfg.lines.graticule, "lon") && cfg.lines.graticule.lon.pos.length > 0) {
    ...
    q.defer(function(callback) {
      d3.json(path + filename("constellations", "borders"), function(error, json) {
        ...
        groups.constBoundaries.selectAll(".bounds")
         .data(conb.features)
         .enter().append("path")
         .attr("class", function(d) { return (Celestial1st.constellation && d.ids.search(re) !== -1) ? "constBoundariesSel" : "constBoundaries"; })
         .attr("d", map);
  ...
  if ((cfg.location || cfg.formFields.location) && cfg.horizon.show && !proj.clip) {
    q.defer(function(callback) {
      var horizon = d3.geo.circle().angle([90]).origin(Celestial1st.nadir());
  ...
  function svgSkyStyle(dist, pt) {
    ...
    var gradient = groups.daylight.append("radialGradient")
     ...
     .attr("fr", "0")
     .attr("r", "300")
     .attr("id", Celestial1st.iId+"skygradient")
    ...
    return {"fill": "url(#"+Celestial1st.iId+"skygradient)", "fill-opacity": skyTransparency(factor, 1.4)}; // KKCHANGE
  }
  ...
  if (!fname) {
    return new Promise(function(resolve, reject) {
      try {
        q.await(function (error) {
          ...
          var svgExp = d3.select("#"+Celestial1st.iId+"d3-celestial-svg svg")
    ...
    q.await(function(error) {
      if (error) throw error;
      var svg = d3.select("#"+Celestial1st.iId+"d3-celestial-svg svg")
      ...
      var a = d3.select("body").append("a").node();
      a.download = fname || "d3-Celestial1st.svg";
      a.rel = "noopener";
      a.href = URL.createObjectURL(blob);
      a.click();
      d3.select(a).remove();
      d3.select("#"+Celestial1st.iId+"d3-celestial-svg").remove();
    });
  }
...
var datetimepicker = function(cfg, callback) {
  ...
  var picker = d3.select("#"+Celestial1st.iId+"celestial-form").append("div").attr("id", Celestial1st.iId+"celestial-date");
  nav("left");
  monSel();
  yrSel();
  nav("right");
  
  var cal = picker.append("div").attr("id", Celestial1st.iId+"cal");
  ...
  function daySel() {
    ...
    for (i=0; i<42; i++) {
      ...
      }).attr("id", Celestial1st.iId+curid)
      .on("click", pick)
  ...
  function yrSel() {     
    picker.append("select").attr("title", "Year").attr("id", Celestial1st.iId+"yr").on("change", daySel);
  ...
  function monSel() { 
    var sel = picker.append("select").attr("title", "Month").attr("id", Celestial1st.iId+"mon").on("change", daySel),
        selected = 0,
        month = date.getMonth();
  ...
  function nav(dir) {
    var lnk = picker.append("div").attr("id", Celestial1st.iId+dir).on("click", function () {
      var mon = $("mon"), yr = $("yr");
  ...
  function timeSel() { 
    picker.append("input").attr("type", "number").attr("id", Celestial1st.iId+"hr").attr("title", "Hours").attr("max", "24").attr("min", "-1").attr("step", "1").attr("value", date.getHours()).on("change", function () { if (testNumber(this) === true) pick(); });

    picker.append("input").attr("type", "number").attr("id", Celestial1st.iId+"min").attr("title", "Minutes").attr("max", "60").attr("min", "-1").attr("step", "1").attr("value", date.getMinutes()).on("change", function () { if (testNumber(this) === true) pick(); });
    
    picker.append("input").attr("type", "number").attr("id", Celestial1st.iId+"sec").attr("title", "Seconds").attr("max", "60").attr("min", "-1").attr("step", "1").attr("value", date.getSeconds()).on("change", function () { if (testNumber(this) === true) pick(); });
  }
  
  function tzSel() { 
    var sel = picker.append("select").attr("title", "Time zone offset from UTC").attr("id", Celestial1st.iId+"tz").on("change", pick),
        selected = 15,
  ...
  this.show = function(dt, tz) {
    ...
    if (nd.offsetTop === -9999) {
      ...
      select("tz", tz);
      set();
      d3.select("#"+Celestial1st.iId+"celestial-date").style({"top": px(top), "left": px(left), "opacity": 1});
      d3.select("#"+Celestial1st.iId+"datepick").classed("active", true);
    } else {
      vanish();
    }
  };
  ...
  this.isVisible = function () {
    if (!document.getElementById(Celestial1st.iId+"datepick")) return false;
    return d3.select("#"+Celestial1st.iId+"datepick").classed("active") === true;
  };
  ...
  function vanish() {
    d3.select("#"+Celestial1st.iId+"celestial-date").style("opacity", 0);
    d3.select("#"+Celestial1st.iId+"error").style( {top:"-9999px", left:"-9999px", opacity:0} );
    d3.select("#"+Celestial1st.iId+"datepick").classed("active", false);
...
this.Celestial1st = Celestial1st;
})();
